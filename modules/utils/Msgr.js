...
const helperFns = {
  ...
  noteNrToStr: (nr) => {
    ...
    if(typeof nr !== 'number') throw new MsgrError('input must be a number');
    if( !(nr > 20 && nr < 109) ) throw new MsgrError('input should be 20 < nr < 109');
  ...
  noteStrToNr: (str) => {
    ...
    if(typeof str !== 'string') throw new MsgrError('input must be a string');
    if(!/^[ABCDEFG](#|)\d$/.test(str)) {throw new MsgrError('Invalid note string ' + str);}

    const key = str.replace(/\d$/, '');
    const nr = keys.indexOf(key);
    if(nr === -1) throw new MsgrError('Invalid key ' + key);
  ...
  getNoteNumber: (nameAndOctave) => {
    ...
    const name = parts[1];
    if(!(name in names)) {throw new MsgrError(`Invalid note name '${nameAndOctave}'`);}
    const octave = parseInt(parts[3]);
    const noteNr = names[name] + (octave * 12);
    if(noteNr<0 || noteNr > 127) {throw new MsgrError('Note nr out of bounds, must be 0-127')}
    return noteNr
  },
...
const channelVoiceMsgs = {
  ...
  ctrl: (channelNr, ctrl_number, value) => {
    if(!isNumber(ctrl_number) || ctrl_number < 0 || ctrl_number > 127) {throw new MsgrError('Ctrl index out of bounds, must be 0-127')}
    if(!isNumber(value) || value < 0 || value > 127) {throw new MsgrError('Ctrl value must be 0-127')}
    const Bn = parseInt('0xB0', 16) + (channelNr - 1);
    if(!isNumber(Bn) || Bn < 176 || Bn > 191) {throw new MsgrError('Ctr Bn index out of bounds, must be 176-191');}
...
const systemCommonMsgs = {
  systemExclusive: () => {
    ...
    throw new MsgrError('Not implemented');
  },
